function sketchProc(p) {
	...
	p.draw = function() {
		...
		fill(255,255,255);
		ellipse(circlePos[0],circlePos[1],circleR*2,circleR*2);
		
		ball.drawIt();
		ball.inAirMove();
		
		//ball.p[0] = circlePos[0] + (circleR-ball.r)*cos(theta);
		//ball.p[1] = circlePos[1] + (circleR-ball.r)*sin(theta);
		//theta+=0.5;
		
		theta = atan((ball.p[1]-circlePos[1])/(ball.p[0]-circlePos[0]));
		//println(theta)
		//line
		var m = -1/tan(theta);
		//println(tan(45));
		...
		ball.resting = false;
		if (sqrt(sq(ball.p[0]-circlePos[0])+sq(ball.p[1]-circlePos[1])) > circleR-ball.r && ball.p[1] > circlePos[1]) {
