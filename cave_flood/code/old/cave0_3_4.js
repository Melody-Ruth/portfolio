function sketchProc(p) {
	...
	var newCircleEnclosure = function(x,y,r) {
		...
		myCircle.collide = function() {
			...
			if (sqrt(sq(ball.p[0]-this.p[0])+sq(ball.p[1]-this.p[1])) > this.r-ball.r && sqrt(sq(ball.p[0]-this.p[0])+sq(ball.p[1]-this.p[1])) < this.r-ball.r+20 && ball.p[1] > this.p[1]) {
				//println(sqrt(sq(ball.p[0]-enclosures[0].p[0])+sq(ball.p[1]-enclosures[0].p[1])) < enclosures[0].r-ball.r+20);
				//this.on = true;
				//println("hi");
			...
			if (sqrt(sq(ball.p[0]-this.p[0])+sq(ball.p[1]-this.p[1])) > this.r-ball.r && sqrt(sq(ball.p[0]-this.p[0])+sq(ball.p[1]-this.p[1])) < this.r-ball.r+20 && ball.p[1] < this.p[1]) {
				//this.on = true;
				//println(sqrt(sq(ball.p[0]-this.p[0])+sq(ball.p[1]-this.p[1])) < this.p[0].r-ball.r+20);
				var unitRamp = [1/sqrt(1+sq(m)),m/sqrt(1+sq(m))];
				var wallTheta = atan(m);
					//Have to adjust because of the radius of the ball.
					//ball.resting = true;//The ball is on a ramp
	...
	var enclosures = [newCircleEnclosure(300,300,200),newCircleEnclosure(1087,300,200)];
	var reverses = [newReverseCircle(300,300,80)];
	var passages = [newPassage(493,250,400,100,false,false,true,true)];
	...
	p.draw = function() {
		...
		if (!ball.inPassage) {
			...
			//testCircle2.collide();
			//testCapsule.collide();
			//testCapsule2.collide();
		}
		//println(enclosures[1].on);
		ball.move();
