...
//Not really working, but I'm saving this since I'm about to do drastic stuff (again)
...
var window = {};
//window.bottomLeft = [canvasWidth/2,0,0];
//window.upperRight = [canvasWidth*3/2,canvasHeight,0];
window.v = [];
window.v[0] = [canvasWidth/2,0,0];
window.v[1] = [canvasWidth*3/2,0,0];
window.v[2] = [canvasWidth*3/2,canvasHeight,0];
window.v[3] = [canvasWidth/2,canvasHeight,0];
//window.fakeX = normalizeVector([window.upperRight[0]-window.bottomLeft[0],0,0]);
//window.fakeY = normalizeVector([0,window.upperRight[1]-window.bottomLeft[1],0]);
window.fakeX = normalizeVector([window.v[1][0]-window.v[0][0],window.v[1][1]-window.v[0][1],window.v[1][2]-window.v[0][2]]);
window.fakeY = normalizeVector([window.v[3][0]-window.v[0][0],window.v[3][1]-window.v[0][1],window.v[3][2]-window.v[0][2]]);
window.fakeZ = normalizeVector([0,0,1]);
...
window.xyz2xy = function(x, y, z, printing) {
	...
	screenPos[1] = canvasHeight/2 - (fakeYPart * this.eyeDistance) / fakeZPart/* - canvasHeight/2*/;
	return screenPos;
}
window.moveIt = function() {
	for (var i = 0; i < this.v.length; i++) {
		this.v[i][0] += player.v[0];
		this.v[i][1] += player.v[1];
		this.v[i][2] += player.v[2];
	}
	//this.fakeZ[0] += player.v[0];
	//this.fakeZ[1] += player.v[1];
	//this.fakeZ[2] += player.v[2];
}
window.myRotate = function() {
	var currAngle;
	var xDiff;
	var yDiff;
	var zDiff;
	...
	if (player.v[3] != 0) {
		for (var i = 0; i < this.v.length; i++) {
			xDiff = this.v[i][0] - player.eyeX;
			yDiff = this.v[i][1] - player.eyeY;
			zDiff = this.v[i][2] - player.eyeZ;
			//fakeXDiff = this.fakeX[0] * xDiff + this.fakeX[1] * this.yDiff + this.fakeX[2] * this.zDiff;
			//fakeYDiff = this.fakeY[0] * xDiff + this.fakeY[1] * this.yDiff + this.fakeY[2] * this.zDiff;
			//fakeZDiff = this.fakeZ[0] * xDiff + this.fakeZ[1] * this.yDiff + this.fakeZ[2] * this.zDiff;
			...
			this.v[i][0] = player.eyeX + radius * cos(currAngle);
			this.v[i][2] = player.eyeZ + radius * sin(currAngle);
		}
		
		radius = 1;
		if (this.fakeZ[0] == 0 && this.fakeZ[2] == 0) {
			currAngle = 0;
			radius = 0;
		} else if (this.fakeZ[0] == 0) {
			if (this.fakeZ[2] > 0) {
				currAngle = 90;
			} else {
				currAngle = 270;
			...
			currAngle = atan(this.fakeZ[2]/this.fakeZ[0]);
			if (this.fakeZ[0] < 0) {
				currAngle += 180;
			}
		}
		currAngle += player.v[3];
		this.fakeZ[0] = radius * cos(currAngle);
		this.fakeZ[2] = radius * sin(currAngle);
	}
	if (player.v[4] != 0) {
		for (var i = 0; i < this.v.length; i++) {
			xDiff = this.v[i][0] - player.eyeX;
			...
			fakeZDiff = this.fakeZ[0] * xDiff + this.fakeZ[1] * this.yDiff + this.fakeZ[2] * this.zDiff;
			console.log(yDiff);
			radius = Math.sqrt(yDiff*yDiff+fakeZDiff*fakeZDiff);
			if (yDiff == 0 && fakeZDiff == 0) {
				...
				currAngle = atan(fakeZDiff/yDiff);
				if (yDiff < 0) {
					currAngle += 180;
				}
			}
			currAngle += player.v[4];
			this.v[i][0] = radius * cos(currAngle) * this.fakeY[0] + radius * sin(currAngle) * this.fakeZ[0] + player.eyeX;
			this.v[i][1] = radius * cos(currAngle) * this.fakeY[1] + radius * sin(currAngle) * this.fakeZ[1] + player.eyeY;
			this.v[i][2] = radius * cos(currAngle) * this.fakeY[2] + radius * sin(currAngle) * this.fakeZ[2] + player.eyeZ;
			console.log(currAngle);
		}
		console.log("Done");
		radius = 1;
		if (this.fakeZ[1] == 0 && this.fakeZ[2] == 0) {
			currAngle = 0;
			radius = 0;
		} else if (this.fakeZ[1] == 0) {
			if (this.fakeZ[2] > 0) {
				currAngle = 90;
			} else {
				currAngle = 270;
			...
			currAngle = atan(this.fakeZ[2]/this.fakeZ[1]);
			if (this.fakeZ[1] < 0) {
				currAngle += 180;
			}
		}
		currAngle += player.v[4];
		this.fakeZ[1] = radius * cos(currAngle);
		this.fakeZ[2] = radius * sin(currAngle);
	}
}
...
console.log(window.xyz2xy(200,100,400));
console.log(xyz2xyOld(200,100,400));
...
function draw() {
	...
	window.fakeY = normalizeVector([window.v[3][0]-window.v[0][0],window.v[3][1]-window.v[0][1],window.v[3][2]-window.v[0][2]]);
	fill(255,0,0);
	ellipse(window.xyz2xy(0,0,150,true)[0],window.xyz2xy(0,0,150,true)[1],10,10);
	if (remoteForward) {
