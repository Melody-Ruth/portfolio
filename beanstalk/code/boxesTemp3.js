...
class Face {
	constructor(newVertices,normalSwitch,myColor1,myColor2,light) {
		...
		this.shouldDrawArray = [];//correspond to vertices. true means should draw. false means shouldn't
		//Not currently using this algorithm
		/*this.lines = [];
		for (var i = 0; i < this.vertices.length - 1; i++) {
			//z = mx + b
			this.lines[i] = [(this.vertices[i+1].z - this.vertices[i].z) / (this.vertices[i+1].x - this.vertices[i].x)];//m
			this.lines[i].push(this.vertices[i].z - this.lines[i][0] * this.vertices[i].x);//b
			//Check which side is inside by comparing with other point. Since convex, all other points should be "inside" this line
			if (i != 0) {
				//We can use 0 to check
				if this.vertices[0] * this.lines[i][0] + this.lines[i][1] < 0) {
					this.lines[i].push(-1);
				} else {
					this.lines[i].push(1);
				}
			} else {
				//We can use this.vertices.length - 1 to check
				if this.vertices[this.vertices.length - 1] * this.lines[i][0] + this.lines[i][1] < 0) {
					this.lines[i].push(-1);
				} else {
					this.lines[i].push(1);
				}
		...
		this.lines[this.vertices.length-1] = [(this.vertices[0].z - this.vertices[this.vertices.length-1].z) / (this.vertices[0].x - this.vertices[this.vertices.length-1].x)];
		this.lines[this.vertices.length-1].push(this.vertices[this.vertices.length-1].z - this.lines[this.vertices.length-1][0] * this.vertices[this.vertices.length-1].x);
		//We can use 1 to check
		if this.vertices[1] * this.lines[this.vertices.length-1][0] + this.lines[this.vertices.length-1][1] < 0) {
			this.lines[this.vertices.length-1].push(-1);
		} else {
			this.lines[this.vertices.length-1].push(1);
		}*/
	}
	...
	checkShouldDraw(printing) {
		...
		for (var i = 0; i < this.vertices.length; i++) {
			if (this.passedFirstPart) {
				...
				&& -check[0] * window.fakeZ[0] + -check[1] * window.fakeZ[1] + -check[2] * window.fakeZ[2] > window.cosMaxAngle) {
					this.shouldDraw = true;
					this.showing[i] = true;
					this.shouldDrawArray[i] = true;
					this.vertices[i].offScreen = false;
				} else {
					this.showing[i] = false;
					this.shouldDrawArray[i] = false;
					this.vertices[i].offScreen = true;
				}
			}
			//Shadow
		...
		if (this.shouldDraw) {
			...
			for (var i = 0; i < this.vertices.length - 1; i++) {
				if (!this.showing[i] && this.showing[i + 1] || this.showing[i] && !this.showing[i + 1]) {
					var badOne;
					if (!this.showing[i]) {
						badOne = i;
					} else {
					...
					var b = [player.eyeX + window.eyeDistance * A, player.eyeY + window.eyeDistance * B, player.eyeZ + window.eyeDistance * C];
					...
					this.shouldDrawArray[badOne] = true;
					this.vertices[badOne].pretendX = v1x + (v2x - v1x) * t;
					this.vertices[badOne].pretendY = v1y + (v2y - v1y) * t;
					this.vertices[badOne].pretendZ = v1z + (v2z - v1z) * t;
					//console.log(this.vertices[i],this.vertices[i+1],"("+this.vertices[badOne].x+", "+this.vertices[badOne].y+", "+this.vertices[badOne].z+")");
				}
			}
			if (!this.showing[this.vertices.length - 1] && this.showing[0] || this.showing[this.vertices.length - 1] && !this.showing[0]) {
				var badOne;
				if (!this.showing[this.vertices.length - 1]) {
					badOne = this.vertices.length - 1;
				} else {
					badOne = 0;
				}
				var A = window.fakeZ[0];
...
class BoxCloud extends Box {
	...
	respondToPlayer() {
		...
		//this.f[0].checkInside([-150,40,700],true);
		//for (var i = 2; i < 3; i++) {
...
function draw() {
	...
	for (var i = allShapes.length - 1; i >= 0; i--) {
		allShapes[i][0] = allShapes[i][1].v[0].x * window.fakeX + allShapes[i][1].v[0].y * window.fakeY + allShapes[i][1].v[0].z * window.fakeZ;
	}
	allShapes.sort(shapeSort);
	for (var i = allShapes.length - 1; i >= 0; i--) {
