...
testPlayer.drawIt = function() {
	stroke(255,0,0);
	strokeWeight(4);
	line(window.xyz2xy(this.p[0],this.p[1],this.p[1])[0],window.xyz2xy(this.p[0],this.p[1],this.p[1])[1],window.xyz2xy(this.p[0],this.p[1]+this.height,this.p[1])[0],window.xyz2xy(this.p[0],this.p[1]+this.height,this.p[1])[1]);
	noStroke();
}
...
class Face {
	...
	respondToPlayer2(printing) {
		...
		testPlayer.p[1] < this.planeXCo*testPlayer.p[0] + this.planeZCo*testPlayer.p[2] + this.planeConstant + 10) {
			this.testPlayerOn = true;
			againstPartV = testPlayer.v[0]*this.normal[0] + testPlayer.v[1]*this.normal[1] + testPlayer.v[2]*this.normal[2];//portion of velocity going against the face (ie portion in the normal direction)
			var againstPartA = testPlayer.a[0]*this.normal[0] + testPlayer.a[1]*this.normal[1] + testPlayer.a[2]*this.normal[2];//portion of acceleration going against the face (ie portion in the normal direction)
			if (againstPartV < 0) {//going towards
				var againstPartVArray = [againstPartV*this.normal[0], againstPartV*this.normal[1], againstPartV*this.normal[2]];
				testPlayer.v = [testPlayer.v[0]-againstPartVArray[0],testPlayer.v[1]-againstPartVArray[1],testPlayer.v[2]-againstPartVArray[2],testPlayer.v[3],testPlayer.v[4]];
				if (this.jumpPlatform) {
			...
			if (againstPartA < 0) {//going towards
				var againstPartAArray = [againstPartA*this.normal[0], againstPartA*this.normal[1], againstPartA*this.normal[2]];
				testPlayer.a = [testPlayer.a[0]-againstPartAArray[0],testPlayer.a[1]-againstPartAArray[1],testPlayer.a[2]-againstPartAArray[2],testPlayer.a[3]];
			}
			testPlayer.p[1] = this.planeXCo*testPlayer.p[0] + this.planeZCo*testPlayer.p[2] + this.planeConstant;
			//Push out just slightly so that you can still see the box
		...
		testPlayer.p[2] < this.planeXCo*testPlayer.p[0] + this.planeYCo*testPlayer.p[1] + this.planeConstant + 10) {
			...
			testPlayer.p[2] = this.planeXCo*testPlayer.p[0] + this.planeYCo*testPlayer.p[1] + this.planeConstant;
			//Push out just slightly so that you can still see the box
			/*testPlayer.p[0] += this.normal[0] * 6;
			testPlayer.p[1] += this.normal[1] * 6;
			testPlayer.p[2] += this.normal[2] * 6;*/
...
class Column extends Shape {
	...
	respondToPlayer(printing) {
		//Change back to normal later
		super.respondToPlayer();
		super.checkCollision2(printing);
	}
}
...
var beanstalk = new Column(20,0,300,50,6000,26);
beanstalk.updateLight([64, 252, 50],[26, 143, 17]);

var testLeaf = new Leaf(-200,20,200,40,Math.PI);
var leaves = [];
var currAngle;
for (var i = 0; i < 15; i++) {
	currAngle = i * (Math.PI / 3 - 0.3);
	leaves[i] = new Leaf(20 + 50 * Math.cos(currAngle), i * 50 + 10, 300 + 50 * Math.sin(currAngle), 20, currAngle);
}
...
function draw() {
	...
	if (goingForward) {
		testPlayer.v[0] = 2*forward[0];
		testPlayer.v[2] = 2*forward[2];
	} else if (goingBack) {
		testPlayer.v[0] = -2*forward[0];
		testPlayer.v[2] = -2*forward[2];
	} else {
	...
	//Change back to normal later
	if (!testPlayer.jumping && testPlayer.p[1] < 10) {
		testPlayer.a[1] = 0;
		testPlayer.v[1] = 0;
		if (testPlayer.p[1] > 0) {
			testPlayer.p[1]--;
